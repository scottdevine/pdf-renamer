...
from pdfrenamer.filename_creators import check_format_is_valid
...
class PDFRenamerGUI:
    ...
    def _create_source_selection(self):
        ...
        # Create a nested frame for the entry and button
        entry_frame = ttk.Frame(source_frame)
        entry_frame.pack(side=tk.RIGHT, fill=tk.X, expand=True, padx=5)

        self.source_var = tk.StringVar()
        source_entry = ttk.Entry(entry_frame, textvariable=self.source_var)
        source_entry.pack(side=tk.LEFT, fill=tk.X, expand=True)

        source_button = ttk.Button(entry_frame, text="Browse...", command=self.browse_source)
    ...
    def _create_dest_selection(self):
        ...
        dest_entry = ttk.Entry(entry_frame, textvariable=self.dest_var)
        dest_entry.pack(side=tk.LEFT, fill=tk.X, expand=True)

        dest_button = ttk.Button(entry_frame, text="Browse...", command=self.browse_dest)
    ...
    def validate_inputs(self):
        ...
        tags = check_format_is_valid(format_str)
        if not tags:
...
def main():
    ...
    root = tk.Tk()
    # Create the application instance (not using the variable directly, but needed to initialize the GUI)
    PDFRenamerGUI(root)
